...
rng(123)
    jjj = 287;
for NT = 287:300 %300; % number of time increments for simulation
    jj = 1;
for xx = XX
    [jjj xx]
    sigma = (1/xx)/(60*60*24);
...
%%
XX = 0.5:0.1:12;
fig8b=figure; p=pcolor(XX,(2:300)/48,min(SEM,0.9));
xlabel('1/\sigma (days)'); ylabel('window length (days)');
colorbar
set(p, 'EdgeColor', 'none');
hold on; plot(XX,(sum(SEM>0.5)+1)/48,'r','linewidth',2)
ylim([2/48 6]);
exportfig(fig8b, 'figures/bootstrap_2.eps', 'width', 16, 'color', 'cmyk','Fontmode','fixed','FontSize', 14);