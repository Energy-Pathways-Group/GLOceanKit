...
function [varargout]=FieldsFromTurbulenceFileWithNamedSSH(file, timeIndex, sshName, varargin)
	ssh = []; sshFD = [];
	rv = []; rvFD = [];
	u = []; uFD = [];
	v = []; vFD = [];
	...
	for iArg=1:length(varargin)
		...
		elseif ( strcmp(varargin{iArg}, 'ssh') )
			ssh = sshFromFile(file, timeIndex, sshName, ssh);
			varargout{iArg} = ssh;
		elseif ( strcmp(varargin{iArg}, 'ssh_fd') )
			[ssh, sshFD]=SSHFDFromFile(file, timeIndex, sshName, ssh, sshFD);
...
function ssh = sshFromFile(file, timeIndex, sshName, ssh)
	if ( isempty(ssh) )
		x = ncread(file, 'x');
		y = ncread(file, 'y');
		ssh = double(ncread(file, sshName, [1 1 timeIndex], [length(y) length(x) 1], [1 1 1]));
	end
end
...
function [ssh, sshFD]=SSHFDFromFile(file, timeIndex, sshName, ssh, sshFD)
