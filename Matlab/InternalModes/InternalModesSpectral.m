...
%             self.rho_zLobatto = InternalModesSpectral.ifct(self.rho_zCheb);
       ...
       if 1 
             self.rho_zCheb = InternalModesSpectral.FitGriddedDensityDataToMonotonicFunctionWithSplines(zIn,rho);
             n = length(zIn);
             if length(self.rho_zCheb) > n
                 self.rho_zCheb = self.rho_zCheb(1:n);
             end
