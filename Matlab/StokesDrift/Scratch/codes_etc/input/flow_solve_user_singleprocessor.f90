...
allocate(temp_u(ny,nz,nx))
allocate(temp_v(ny,nz,nx))
! reading in ascii data from matlab
   open(10,file='input/cyprus_um.txt',status='old',form='formatted')
  write(0,*) 'about to read data'
 read(10,*) (temp_u(:,k,:),k=1,nz-1)
 write(0,*)'done with first data'
 close(unit=10)
 open(11,file='input/cyprus_vm.txt',status='old',form='formatted')
 read(11,*) (temp_v(:,k,:),k=1,nz-1)
 close(unit=11)
