classdef ForcedDissipativeQGPVE < QGPVE
    properties
        k_f
        j_f
        EMA0
    ...
    methods
        function self = ForcedDissipativeQGPVE(wvt,options)
            arguments
                ...
                options.j_f (1,1) double = 1 % forcing mode---ignored for a 2D simulation
            ...
            self@QGPVE(wvt,fluxName='ForcedDissipativeQGPVE',r=r,nu=nu,shouldUseBeta=options.shouldUseBeta,stateVariables=fluxVar);
            ...
            self.EMA0(wvt.Kh > self.k_f-deltaK/2 & wvt.Kh < self.k_f+deltaK/2 & wvt.J == self.j_f) = 0;
            ...
            if strcmp(options.initialPV,'narrow-band') || strcmp(options.initialPV,'full-spectrum')
                ...
                for iK=1:(length(wvt.kRadial)-1)
                    indicesForK = find( kAxis(iK)-dk/2 <= wvt.Kh & wvt.Kh < kAxis(iK)+dk/2 & wvt.J == self.j_f  );
                ...
                if strcmp(options.initialPV,'narrow-band')
                    ...
                    fprintf('desired energy: %g, actual energy %g\n',0.5 * u_rms^2,wvt.geostrophicEnergy/wvt.Lz);
                end
            end
        ...
        function varargout = compute(self,wvt,varargin)
            ...
            [varargout{:}] = compute@QGPVE(self,wvt,varargin{:});
            F0 = varargout{1};
            F0_psi = (~self.EMA0) .* F0;
            forcing = wvt.transformToSpatialDomainWithF(A0=self.PVA0 .* F0_psi);
            F0 = self.EMA0 .* F0;
            varargout{1} = F0;
        ...
        function writeToFile(self,ncfile,wvt)
            ...
            writeToFile@QGPVE(self,ncfile,wvt);
            ncfile.addAttribute('j_f',self.j_f)
        ...
        function flag = isequal(self,other)
            ...
            flag = isequal@QGPVE(self,other);
            flag = flag & isequal(self.j_f,other.j_f);
    ...
    methods (Static)
        function nlFlux = nonlinearFluxFromFile(ncfile,wvt)
            ...
            nlFlux = ForcedDissipativeQGPVE(wvt,initialPV='none', j_f=ncfile.attributes('j_f'), k_f=ncfile.attributes('k_f'),r=ncfile.attributes('r'),nu=ncfile.attributes('nu') );
        end
    end
